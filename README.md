...
训练过程会在`models/{模型名称}/`目录下生成多个模型权重文件：

| 文件名 | 含义 | 保存规则 | 推荐用途 |
|-------|------|---------|---------|
| `best_epoch_weights.pth` | **验证集损失最低**的模型 | 自动保存验证集表现最佳的轮次 | ✅ **推荐部署** - 泛化能力最强 |
| `last_epoch_weights.pth` | 最后一轮的模型 | 每轮覆盖更新 | 训练中断恢复 |
| `epXXX-lossX.XXX-val_lossX.XXX.pth` | 特定轮次的检查点 | 每`save_period`轮保存一次 | 回溯分析、防过拟合 |
| `best_minority_score_model.pth` | 少数类别优化的模型 | 少数类准确率最高时保存 | 类别不平衡场景 |

**文件名解读示例**:
```
ep050-loss0.157-val_loss0.296.pth
│   │       │           │
│   │       │           └─ 验证集损失值 (0.296)
│   │       └─────────────── 训练集损失值 (0.157)
│   └───────────────────────── 训练轮次 (第50轮)
...
**选择指南**:
- **生产部署**: 使用`best_epoch_weights.pth` (默认选项)
- **训练恢复**: 使用最新的`epXXX-lossX.XXX-val_lossX.XXX.pth`
- **过拟合调试**: 对比不同轮次的检查点，选择val_loss较低的
- **类别不平衡任务**: 优先考虑`best_minority_score_model.pth`

> **注意**: 训练集loss很低但val_loss很高说明过拟合，应选择更早的检查点
