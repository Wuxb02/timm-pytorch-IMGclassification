...
def plot_patient_curves(metrics_folder: str):
    """
    对指定的 metrics_out 子文件夹绘制患者级别ROC和PR曲线
    ...
    print("=" * 80)
    print(f"患者级别ROC/PR曲线绘制: {metrics_folder}")
    print("=" * 80)
    ...
    # 5. 调用绘图函数
    print("\n正在绘制ROC曲线...")
    roc_output = os.path.join(folder_path, "patient_roc_curves.png")
    roc_auc = draw_roc_curves(labels, probs, class_names, roc_output)
    print(f"✓ ROC曲线已保存: {roc_output}")

    print("\n正在绘制PR曲线...")
    pr_output = os.path.join(folder_path, "patient_pr_curves.png")
    ap = draw_pr_curves(labels, probs, class_names, pr_output)
    print(f"✓ PR曲线已保存: {pr_output}")
